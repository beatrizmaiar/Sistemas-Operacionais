 ...
 */
  #define IDLE_QUEUE 15
  #define USER_QUEUE_START 7
  register struct proc *rp;			/* process a executar */
  struct proc **rdy_head;	/* primeiro processo de uma dada fila */
  int p, q;				/* itera sobre processos e filas */
  int tickets, chosen_ticket;     /* tickets totais e ticket escolhido */
  int ready_procs[NR_SCHED_QUEUES] = {0};      /* o número de processos prontos para serem executados em uma dada fila */
  int tickets_in_queue[NR_SCHED_QUEUES] = {0};    /* o número de tickets em uma dada fila */
  
  rdy_head = get_cpulocal_var(run_q_head);

  for (q = 0; q < USER_QUEUE_START; q++) {
	/* Escalonamento padrão do MINIX 3 para processos de sistema */
	if(!(rp = rdy_head[q])) {
  ...
  for (p = 0; p < NR_TASKS + NR_PROCS; p++) {
	/* Conta o número de processos de usuário em cada fila */
	struct proc *current;
	current = &proc[p];
	if (current->p_priority >= USER_QUEUE_START && proc_is_runnable(current)) {
		ready_procs[current->p_priority]++;
	}
  }
  
  tickets = 0;
  
  for (q = USER_QUEUE_START; q < IDLE_QUEUE; q++) {
	/* Conta o número de tickets */
	int weight = IDLE_QUEUE - q;
	int cont = ready_procs[q] * weight;
	
	tickets += cont;
	tickets_in_queue[q] = cont;
  }

  if (tickets > 0) {
	/* Existem processos de usuário disponíveis */
	chosen_ticket = 1 + (random() % tickets);
	
	for (q = USER_QUEUE_START; q < IDLE_QUEUE; q++) {
		/* Percorre as filas buscando aquela em que o ticket escolhido se encontra */

		int weight = IDLE_QUEUE - q;

		if (chosen_ticket > tickets_in_queue[q]) {
			/* O ticket escolhido não está nessa fila */
			chosen_ticket -= tickets_in_queue[q];	/* "paga" o número de tickets na fila para avançar para a próxima */
		}
		else {
			/* O ticket escolhido está nessa fila */
			rp = rdy_head[q];
			while (chosen_ticket > weight) {
				/* Percorre a fila até encontrar o processo sorteado */
				chosen_ticket -= weight;	/* "paga" o peso do processo para avançar para o próximo */
				rp = rp->p_nextready;
			}
			assert(proc_is_runnable(rp));
			if (priv(rp)->s_flags & BILLABLE)
  ...
  /* Se não houver nenhum processo disponível, retorna NULL */
  return NULL;
}
